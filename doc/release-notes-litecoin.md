flight Core version *0.15.1* is now available from:

  <https://download.flight.org/flight-0.15.1/>
...
A number of changes to the way flight Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 legs per kB (0.00001 myPPL). This is to prepare for dropping the minimum transaction fee to 0.1 legs per kB (0.0001 myPPL) in the next release.
